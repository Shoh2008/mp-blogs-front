      ...
      <Navbar>
        <div>
          <div className="image">
            <div className="img">
              {
                user.image ? <img src={"https://mp-blogs-api.onrender.com/files/" + user.image} /> : user?.username?.charAt(0)?.toUpperCase()
              }
            </div>
          </div>
          <h3>{user.username}</h3>
        ...
        {blogs?.map((item, index) => <div className="blog" key={index}>
          <div className="user">
            <div className="image">
              <div className="img">
                {
                  item.author_image ? <img src={"https://mp-blogs-api.onrender.com/files/" + item.author_image} /> : item?.author?.charAt(0)?.toUpperCase()
                }
              </div>
            </div>
            <h3>{item.author}</h3>
          ...
          <div className="blog-img">
            {
              item.image ? <img src={"https://mp-blogs-api.onrender.com/files/" + item.image} /> : "🎃"
            }
          </div>
          <h3 className="title">{item.title}</h3>
          <div>
            <audio src={"https://mp-blogs-api.onrender.com/files/" + item.audio} controls></audio>
          </div>
          {
